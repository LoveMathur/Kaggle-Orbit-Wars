...
class ProducerLiteConfig:
    ...
    roi_threshold: float = 1.30              # fire if score > this
    ...
    reinforce_size_beta: float = 0.80
...
def _apply_safe_terminal_config(
    ...
    *,
    ...
    """Continuous ROI adaptation — replaces the 60-turn hard window.

    The original version only activated in the final 60 turns (12% of the game),
    leaving the ROI gate unchanged for 88% of turns even when the agent was visibly
    losing. This version uses production share relative to expected share to detect
    deficits continuously and lower the gate proportionally throughout the game.

    The 'terminal_phase_turns' field in config is intentionally left unused here —
    it is still needed by _apply_safe_terminal_candidate_adjustments for the ETA
    filter (which correctly gates only in the final 60 turns).
    """
    if int(player_count) <= 0 or int(obs.P) <= 0:
        return config
    ...
    strength = _owner_strength(
        obs, prod, player_count=int(player_count), dtype=dtype, device=device
    )
    if pid < 0 or pid >= int(player_count) or strength.numel() == 0:
        return config

    my_strength = float(strength[pid].item())
    total_strength = float(strength.sum().clamp(min=1e-6).item())

    # Production-weighted share of the total economy.
    # Expected in a fair game = 1 / player_count.
    my_share = my_strength / total_strength
    expected_share = 1.0 / max(1, int(player_count))
    # relative > 1.0 = ahead; < 1.0 = behind.
    relative = my_share / max(expected_share, 1e-9)

    if relative >= 1.10:       # comfortably ahead: no adjustment
        scale = 1.00
    elif relative >= 0.90:     # roughly even: slight loosening
        scale = 0.95
    elif relative >= 0.70:     # behind: meaningful relief
        scale = 0.87
    elif relative >= 0.50:     # significantly behind: strong relief
        scale = 0.78
    else:                      # critically behind: maximum relief
        scale = 0.70

    new_roi = max(1.10, float(config.roi_threshold) * scale)

    # One extra wave when behind, capped at 8 total.
    extra_waves = 1 if relative < 0.90 else 0
    new_waves = min(8, int(config.max_waves_per_turn) + extra_waves)

    # Avoid creating a new dataclass object when nothing changed (fast path).
    roi_unchanged = abs(new_roi - float(config.roi_threshold)) < 1e-6
    waves_unchanged = new_waves == int(config.max_waves_per_turn)
    if roi_unchanged and waves_unchanged:
        return config

    return dataclasses.replace(
        config,
        roi_threshold=new_roi,
        max_waves_per_turn=new_waves,
        enable_regroup=True,
    )
...
def plan_lite_waves(
    *,
    ...
    if beta > 0.0:
        ...
        # Hard cap: floor inflation from enemy risk cannot exceed 2× the target's
        # current garrison. Prevents distant enemy mass from making nearby
        # neutrals permanently unreachable regardless of beta value.
        _base_garrison = garrison_status.ships[
            target_idx.clamp(0, P - 1), 0
        ].to(dtype).clamp(min=1.0).view(T, 1)
        reinforcement = reinforcement.clamp(max=_base_garrison * 2.0)
    floor = capture_floor(
...
def _warmup() -> None:
    """Pre-compile the PyTorch path that causes the consistent ~300ms spike
    at t≈97-142 in all replays.

    Turn-0 JIT covers the zero-fleet tensor shapes. A second compilation
    fires when fleet count first reaches ~20, producing a 300ms pause mid-game.
    This warmup exercises that shape during import so both compilations are
    absorbed into the turn-0 overage budget.
    """
    _dummy_planets = [
        [0, 0, 15.0, 15.0, 2.3, 50, 3],
        [1, 1, 85.0, 85.0, 2.3, 50, 3],
        [2, -1, 30.0, 70.0, 1.7, 20, 2],
        [3, -1, 70.0, 30.0, 1.7, 20, 2],
        [4, -1, 15.0, 85.0, 2.0, 12, 2],
        [5, -1, 85.0, 15.0, 2.0, 12, 2],
    ]
    # 22 fleets — enough to cross the shape boundary seen in replays at t≈100-140
    _dummy_fleets = [
        [i, 1, 50.0, 50.0, 0.5, 1, 8] for i in range(22)
    ]
    _obs_heavy = {
        "player": 0,
        "planets": _dummy_planets,
        "fleets": _dummy_fleets,
        "angular_velocity": 0.035,
        "comet_planet_ids": [],
        "remainingOverageTime": 59.0,
        "initial_planets": _dummy_planets,
    }
    try:
        with torch.no_grad():
            agent(_obs_heavy)
